...
def test_with_gas_dec_reading_single(settings):
    ...
    result = GasReading.objects.with_dec_reading().get()
...
def test_with_gas_dec_reading_cumulative(settings):
    ...
    qs = GasReading.objects.with_dec_reading().order_by("reading_date")
...
def test_with_gas_dec_diff(settings):
    ...
    result = GasReading.objects.with_dec_reading().with_dec_diff().get()
...
def test_with_gas_dec_sum():
    ...
    result = GasReading.objects.with_dec_sum().get()
...
def test_with_gas_dec_sum_multiple_records():
    ...
    qs = GasReading.objects.with_dec_sum().order_by("reading_date")
...
@pytest.mark.django_db
def test_with_gas_dec_sum_with_positive_adj_costs():
    GasReadingFactory(
        reading_date=date(2024, 1, 1), reading_qty=10, unit_price=Decimal("7.50"), adj_costs=Decimal("20.00")
    )

    result = GasReading.objects.with_dec_sum().get()

    assert result.gas_dec_sum == Decimal("95.00")  # 10 * 7.50 + 20.00


@pytest.mark.django_db
def test_with_gas_dec_sum_with_negative_adj_costs():
    GasReadingFactory(
        reading_date=date(2024, 1, 1), reading_qty=10, unit_price=Decimal("7.50"), adj_costs=Decimal("-20.00")
    )
    ...
    assert result.gas_dec_sum == Decimal("55.00")  # 10 * 7.50 - 20.00


@pytest.mark.django_db
def test_gas_reading_default_adj_trans_cost():
    reading = GasReadingFactory(reading_date=date(2024, 1, 1))

    assert reading.adj_trans_cost == Decimal("0")


@pytest.mark.django_db
def test_gas_reading_positive_adj_trans_cost():
    reading = GasReadingFactory(reading_date=date(2024, 1, 1), adj_trans_cost=Decimal("15.00"))

    assert reading.adj_trans_cost == Decimal("15.00")


@pytest.mark.django_db
def test_gas_reading_negative_adj_trans_cost():
    reading = GasReadingFactory(reading_date=date(2024, 1, 1), adj_trans_cost=Decimal("-10.00"))

    assert reading.adj_trans_cost == Decimal("-10.00")
...
def test_queryset_chaining(settings):
    ...
    result = GasReading.objects.with_dec_reading().with_dec_diff().with_dec_sum().get()
